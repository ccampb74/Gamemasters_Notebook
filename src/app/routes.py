...
from app.forms import SignUpForm, SignInForm, CampaignForm, NoteForm, CharacterForm, CharacterEditForm, SessionForm, \
    SessionEditForm
    ...
    if form.validate_on_submit():
        ...
        if user:
            if bcrypt.checkpw(hashed_passwd, user.password):
                ...
                return render_template('users_sign_in.html', title=app.config['USER SIGNIN'], user=current_user,
                                       form=form, return_message=return_message)

            return redirect(url_for('index'))
        else:
            return_message = "User ID not recognized!"

            return render_template('users_sign_in.html', title=app.config['USER SIGNIN'], user=current_user, form=form,
...
def users_sign_up():
    ...
    if form.validate_on_submit():
        ...
        if passwd == passwd_confirm:
            ...
            return render_template('users_sign_up.html', title=app.config['USER SIGNUP'], user=current_user, form=form,
                                   return_message=return_message)
...
def campaign(id):
    ...
    if current_user.is_authenticated:
        if current_user.id == campaign_game_master_id:
            ...
            return render_template('campaign_display.html',
                                   user=current_user, send_private_note=send_private_note,
                                   private_notes=private_notes, private_note_form=private_note_form, campaign=campaign,
